 ...
 * to execute an {@link CoinsBonus} action controlling its validation.<br>
 * Implement {@link Serializable}
 * 
 * @author Stefano
 *
 */
public class CoinsBonusRqst extends BonusRqst {

	private static final long serialVersionUID = 1L;

	/**
	 * Construct a CoinsBonusRqst using the super constructor
	 * 
	 ...
	 */
	public CoinsBonusRqst(Token token, Bonus bonus) {
		super(token, bonus);
	}
	...
	protected boolean isValid(Game game, Player player) throws IllegalActionException {
		...
		/*
		 * Control if the bonus is a QuantityBonus and the name must be equals to COINSBONUS
		 */
		if(!("COINSBONUS".equals(getBonus().getName()) && getBonus() instanceof QuantityBonus)) 
