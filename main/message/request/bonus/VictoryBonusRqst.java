 ...
 * to execute an {@link VictoryBonus} action controlling its validation.<br>
 * Implement {@link Serializable}
 * 
 * @author Stefano
 *
 */
public class VictoryBonusRqst extends BonusRqst {

	private static final long serialVersionUID = 1L;

	/**
	 * Construct a VictoryBonusRqst using the super constructor
	 * 
	 ...
	 */
	public VictoryBonusRqst(Token token, Bonus bonus) {
		super(token, bonus);
	}
	...
	protected boolean isValid(Game game, Player player) throws IllegalActionException {
		...
		/*
		 * Control if the bonus is a QuantityBonus and the name must be equals to VICTORYBONUS
		 */
		if(!("VICTORYBONUS".equals(getBonus().getName()) && getBonus() instanceof QuantityBonus)) 
